...
export function decorateLeads(leads) {
  return leads.map((l) => {
    ...
    const reach = REACH_VIA_CHANNELS.filter((c) => l[c.column]).map(
      (c) => c.key,
    );
...
export function buildLeadFilterOptions(decorated, clientId) {
...
export function buildCategoryCounts(decorated, clientId) {
  const counts = {};
  decorated.forEach((l) => {
  ...
  return getClientLeadCategoryTypes(clientId)
