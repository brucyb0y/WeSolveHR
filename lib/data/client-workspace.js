...
// Performance-tab metrics: GTM velocity and inactivity alerts, all derived from
// lead and meeting timestamps rather than stored anywhere.
//
// `nowMs` is injected rather than read from Date.now() inside, so every figure
// on one render measures against a single instant — otherwise a slow render
// could compute "days since last lead" against a different millisecond than
// "days since last demo" and the two cards would disagree at a day boundary.
//
// daysSince* return null (not 0) when nothing has ever been recorded: the
// Performance tab renders null as "—", where 0 would claim it happened today.
export function buildClientPerformanceMetrics({
  leadAllRows = [],
  meetings = [],
  nowMs = Date.now(),
}) {
  const dayMs = 24 * 60 * 60 * 1000;
  const createdMs = (l) => (l.created_at ? new Date(l.created_at).getTime() : 0);

  const leadsLast3 = leadAllRows.filter(
    (l) => createdMs(l) >= nowMs - 3 * dayMs,
  ).length;
  const leadsLast7 = leadAllRows.filter(
    (l) => createdMs(l) >= nowMs - 7 * dayMs,
  ).length;
  ...
  const lastLeadMs = leadAllRows.reduce(
    (max, l) => Math.max(max, createdMs(l)),
    0,
  );
  const daysSinceLastLead = lastLeadMs
    ? Math.floor((nowMs - lastLeadMs) / dayMs)
    : null;

  // Meetings prefer meeting_date but fall back to created_at, so a meeting
  // logged without a date still counts as recent activity.
  ...
  const daysSinceLastMeeting = lastMeetingMs
    ? Math.floor((nowMs - lastMeetingMs) / dayMs)
    : null;
    ...
    alerts.push(
      `No new leads in ${
        daysSinceLastLead === null
          ? "the recorded period"
          : `${daysSinceLastLead} day${daysSinceLastLead === 1 ? "" : "s"}`
      } — GTM inactivity`,
    );
  }
  if (leadsLast7 === 0) {
    alerts.push("No weekly progress: 0 leads added in the last 7 days");
  }
  ...
  return {
    ...
    totalLeads: leadAllRows.length,
  };
}

// ---------------------------------------------------------------------------
// Lead filter option lists.
//
// These were locals inside the page renderer, declared next to the markup that
// consumed them. They live here now because buildLeadFilters() above reads the
// same query keys: if an option list and the parser disagree about a key, the
// filter silently does nothing. Keeping both in one file makes that visible.
// ---------------------------------------------------------------------------

export const HAS_PHONE_FILTER_OPTIONS = [
  { key: "yes", label: "Yes" },
  { key: "no", label: "No" },
];
...
export const NOTE_AUDIO_FILTER_OPTIONS = [
  { key: "yes", label: "Has audio" },
  { key: "no", label: "No audio" },
];

export const MISSED_CALLBACK_FILTER_OPTIONS = [
  { key: "yes", label: "Yes — overdue (past)" },
  { key: "no", label: "No — upcoming (future)" },
  { key: "none", label: "No callback date set" },
];

// "both" is not a channel — it matches leads reached via LinkedIn AND email,
// which is why this cannot simply be REACH_VIA_CHANNELS.
export const buildReachedViaFilterOptions = (channels) => [
  { key: "__none__", label: "None (not reached)" },
  ...channels.map((c) => ({ key: c.key, label: c.label })),
