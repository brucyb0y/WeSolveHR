...
// Session-ONLY auth, with no basic-auth fallback.
//
// Use this for routes that act on "the caller" rather than on org data —
// /api/account/profile-field writes to `user.id`, so falling back to the
// dashboard admin would let anyone holding the shared DASHBOARD_PASSWORD edit
// that admin's profile. The Express original enforced this by using
// requireUserLogin instead of requireDashboardAuth; this is the equivalent.
//
// Returns { user } or { response } holding a 401.
export async function requireSessionUser() {
  const user = await getSessionUser();
  if (!user) return { response: apiError(401, "Not authenticated") };
  return { user };
}
