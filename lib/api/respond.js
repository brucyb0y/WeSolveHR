...
// Multipart helper: pull an uploaded file out of a FormData and give it the
// shape the existing helpers expect.
//
// The Express handlers used multer, which produced
// `{ buffer, originalname, mimetype }`. Native route handlers get a web `File`
// instead, so the buffer has to be materialised explicitly — the storage and
// transcription helpers in lib/server/app.js take a Node Buffer and cannot
// consume a stream.
//
// Returns null when the field is absent or not a file, so callers can 400
// with their own message.
export async function readUploadedFile(form, field) {
...
// Form fields as a plain object, so handlers written against `req.body` read
// the same way. Repeated keys collapse to arrays, matching Express.
export function formToBody(form) {
  const out = {};
  for (const [k, v] of form.entries()) {
    if (typeof v?.arrayBuffer === "function") continue; // skip files
    if (k in out) out[k] = Array.isArray(out[k]) ? [...out[k], v] : [out[k], v];
    else out[k] = v;
  }
  return out;
