...
// The session store MUST hang off globalThis, not a module-level binding.
//
// Next bundles server components / server actions and route handlers into
// SEPARATE module graphs, so this file is instantiated more than once in one
// process (verified: two "module init" hits per boot). With a plain
// `const store = new Map()` each graph gets its own store — login (a server
// action) writes to one, and the Express-adapter API routes read another that
// is always empty. The page renders logged-in while every /api/* call 302s to
// /login, and a client-side fetch then tries to JSON.parse the login HTML.
//
// globalThis is shared across those graphs, so keying the store there gives
// every instance the same Map.
const store = (globalThis.__wesolveSessionStore ||= new Map());
