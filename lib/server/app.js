...
function mapExcelRowToClientLead(row) {
  ...
  const phoneRaw = get(
    ...
    // "Company Number" is deliberately NOT a fallback here — it is a distinct
    // field that lands in company_hq_phone. Letting phone borrow it conflated
    // the two whenever a row had no Persona Number, so both showed the same
    // value. A row with no persona number simply has no phone.
    "Persona Number",
    "persona_number",
    "Phone Number",
    "Phone numbers",
