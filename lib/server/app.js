import { createCollector, createUpload } from "./adapter.js";
...
async function transcribeAudioBuffer({ buffer, contentType, fileName, prompt }) {
  if (!openai) {
...
function buildBusinessLeadPayloadFromBody(body) {
  return {
    ...
    ...(body.pipeline_stage !== undefined
      ? { pipeline_stage: normalizeText(body.pipeline_stage || "prospect_identified") }
      : {}),
    ...
    ...(body.phone_assigned_to !== undefined
      ? { phone_assigned_to: String(body.phone_assigned_to || "").trim() || null }
      : {}),
    ...(body.email_assigned_to !== undefined
      ? { email_assigned_to: String(body.email_assigned_to || "").trim() || null }
      : {}),
...
function mapExcelRowToClientLead(row) {
  ...
  // "Work Email" is the Rebus AI sheet header; other sheets use "Email".
  const emailRaw = get("Work Email", "Email", "email");
  const email = firstLine(emailRaw);
  const teamSize = get(
  ...
  const splitCity = commaAt === -1 ? location : location.slice(0, commaAt).trim();
  ...
  const phoneRaw = get(
    ...
    "Mobile Number",
    "mobile",
    // Revivflow's header is the bare "Number". Kept last so a sheet that has
    // both a real phone column and a "Number" column still prefers the former.
    "Number",
  );
  const phoneFirst =
    String(phoneRaw)
  ...
  pushNote("Recent Activity (LinkedIn)", "Recent activity linkedIn", "recent_activity_linkedin");
  pushNote("Agency / Audience Size", "Agency Size Audience size", "agency_size_audience_size", "Audience size", "Agency size");
  ...
  return {
    ...
    company_instagram_url:
      get("Company Instagram URL", "company_instagram_url", "Instagram Link") ||
      null,
    company_youtube_url:
    ...
    company_hq_phone: get("Company HQ Phone", "company_hq_phone") || null,
    ...
    company_subtype: get("Company Subtype", "company_subtype") || null,
    ...
    verified_by:
      get("verified by", "verified_by", "Verified") || null,

    lead_category: "b2b",
    lead_stage: "prospect",
    lead_source: normalizeText(get("lead_source", "source", "Sources") || "excel"),
    status: "new",
    notes: noteLines.join("\n"),
    enrichment_status: "imported",
      ...
      try {
        ...
        if (insertError) {
          ...
          if (insertError.code === "23505") {
            results.duplicates += 1;
            if (phoneKey) seenPhoneKeys.set(phoneKey, seenPhoneKeys.get(phoneKey) || null);
...
function buildClientReportStatsForAI(
  ...
  const leadsFound = Math.trunc(enrichedLeads * LEADS_FOUND_ENRICHED_MULTIPLIER);
  ...
  workItems.forEach((w) => {
    ...
    if (taskDetails.length < 15) {
      ...
      taskDetails.push({
        title,
        description: String(w.description || "").trim().slice(0, 300) || null,
  ...
  linkedTasks.forEach((t) => {
    ...
    if (taskDetails.length < 15) {
      taskDetails.push({
        title,
        description: String(t.detail || "").trim().slice(0, 300) || null,
  ...
  const strategySessionsCount = meetingsInWindow.filter(isStrategyMeeting).length;
...
async function generateClientReportSummaryStructured({ clientName, period, stats }) {
  if (!openai) {
    throw new Error(
      "OPENAI_API_KEY is missing. Add it in Railway variables first.",
    );
  ...
  const [
    ...
    linkedTasksR,
      ...
      supabase
        ...
        .select("id, meeting_date, meeting_type, duration_min, created_at, created_by_user_id")
        .eq("org_id", orgId)
        .eq("client_id", clientId)
        .eq("is_active", true)
        .is("deleted_at", null),
...
async function runClientReportSummary({
  ...
  const data =
    ...
    (await fetchClientReportInputsForAI(
      ...
      period === "weekly" && weekStartMs ? mondayStartOfUtcMs(weekStartMs) : null,
      client,
    ));
...
async function runDailyClientReportSummaries() {
  ...
  for (const client of clients || []) {
    ...
    try {
      ...
      failed += 1;
      console.error(
        `[report-summary] client ${client.id} failed:`,
        e.message,
      );
    }
  }
...
app.post("/api/cron/generate-report-summaries", async (req, res) => {
  try {
    const secret = process.env.CRON_SECRET;
    const provided =
      req.get("x-cron-secret") || req.query.secret || req.body?.secret;
    if (!secret || provided !== secret) {
      return sendApiError(res, 401, "Unauthorized");
    }
    const result = await runDailyClientReportSummaries();
    return sendApiSuccess(res, result);
  } catch (error) {
    console.error("POST /api/cron/generate-report-summaries error:", error);
    return sendApiError(
      res,
      500,
      error.message || "Failed to generate report summaries",
    );
  }
});
...
app.post(
  "/api/clients/:id/report-summary/generate",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      const clientId = Number(req.params.id);
      if (!clientId) return sendApiError(res, 400, "Invalid client id");
      const period = CLIENT_REPORT_SUMMARY_PERIODS.includes(req.body?.period)
        ? req.body.period
        : "daily";
      // Optional week_start (YYYY-MM-DD Monday) targets a specific week's summary
      // when regenerating from a historical Week N tab. Ignored for daily.
      let weekStartMs = null;
      if (period === "weekly" && req.body?.week_start) {
        const parsed = Date.parse(`${String(req.body.week_start).slice(0, 10)}T00:00:00Z`);
      ...
      const { data: client, error } = await supabase
        ...
        .maybeSingle();
      if (error) throw error;
      if (!client) return sendApiError(res, 404, "Client not found");
      const saved = await runClientReportSummary({
        orgId,
        client,
        period,
        userId: req.loggedInUser?.id || null,
        weekStartMs,
      });
      return sendApiSuccess(res, saved);
    } catch (error) {
      console.error(
        "POST /api/clients/:id/report-summary/generate error:",
        error,
      );
      return sendApiError(
        res,
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const clientId = Number(req.params.id);
      if (!clientId) return sendApiError(res, 400, "Invalid client id");
      const rawItems = Array.isArray(req.body?.goals_json) ? req.body.goals_json : [];
          ...
          [
            {
              ...
              notes: notes,
              updated_by_user_id: req.loggedInUser?.id || null,
              updated_at: now,
            },
          ],
      ...
      await insertClientActivityLog({
        orgId,
        clientId,
        actorUserId: req.loggedInUser?.id || null,
      ...
      return sendApiError(res, 500, error.message || "Failed to save goals");
    }
  },
);
  ...
  try {
    ...
    const [plannedOffRows, usersResult, eventsResult] = await Promise.all([
      ...
      supabase
        ...
        .eq("is_active", true)
        .order("name", { ascending: true }),
      // Only need each user's latest event for *today* to know who's on break;
      // scoping to the current attendance day avoids scanning the whole table.
      supabase
        .from("attendance_events")
        .select("user_id, action, created_at")
        .eq("org_id", actingUser.org_id)
...
app.post("/api/account/profile-field", requireUserLogin, async (req, res) => {
  try {
    ...
    if (!options) {
      return sendApiError(res, 400, "Invalid field");
    }
    ...
    console.error("POST /api/account/profile-field error:", error);
    return sendApiError(res, 500, error.message || "Failed to update profile field");
  }
});
...
app.get("/api/clients/nav-list", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const { data, error } = await supabase
      .from("clients")
    ...
    return sendApiError(res, 500, "Failed to load clients");
  }
});
...
async function importClientLeadsFromExcel({
  ...
  try {
    for (let i = 0; i < rows.length; i += 1) {
      ...
      try {
        ...
        if (phoneKey && seenPhoneKeys.has(phoneKey)) {
          duplicateOfId = seenPhoneKeys.get(phoneKey);
          duplicateMessage = "Duplicate phone number skipped";
        } else if (!dedupeOnEmail && websiteKey && seenWebsiteKeys.has(websiteKey)) {
        ...
        results.errors.push({ row: rowNumber, error: error.message || String(error) });
        await addLeadImportRowLog({
          importId: importLog.id,
          orgId,
          business,
    ...
    await finishLeadImportLog({ importId: importLog.id, results, status: "completed" });
    return results;
  } catch (error) {
    await finishLeadImportLog({
      importId: importLog.id,
...
app.post(
  "/api/clients/:id/leads/import-excel",
  requireDashboardAuth,
  upload.single("file"),
  async (req, res) => {
    try {
      ...
      if (!clientId) return sendApiError(res, 400, "Invalid client id");
      if (!req.file?.buffer) return sendApiError(res, 400, "Excel file is required");

      // Category type picked in the import dialog, applied to every sheet row.
      const categoryType = String(req.body?.category_type || "").trim();
      if (
        categoryType &&
        !CLIENT_LEAD_CATEGORY_TYPES.some((c) => c.key === categoryType)
      ) {
        return sendApiError(res, 400, "Invalid category type");
      }

      // A client mapped to an inline static business (e.g. Rasset) imports into
      // that business's own table using its sheet mapper, mirroring the
      // standalone /leads/<business> import; everyone else imports into their
      // per-client client_leads rows.
      ...
      // A rejected sheet (e.g. an "Assigned to" name that is not a user) carries
      // its own 400 + row-level message and is a normal user-facing outcome, not
      // a server fault.
      const status = error.statusCode || 500;
      if (status >= 500) {
        console.error("POST /api/clients/:id/leads/import-excel error:", error);
      }
      return sendApiError(res, status, error.message || "Failed to import leads");
    }
  },
);
...
app.post(
  "/api/clients/:id/leads/:leadId/call-recording",
  requireDashboardAuth,
  upload.single("audio"),
  async (req, res) => {
    try {
      ...
      if (!clientId || !leadId)
        return sendApiError(res, 400, "Invalid client or lead id");
      if (!req.file?.buffer)
        return sendApiError(res, 400, "Audio file is required");

      const recordingUrl = await uploadLeadCallAudio(
        req.file.buffer,
        req.file.originalname || "call.mp3",
      );
      ...
      let recordingQuery = supabase
        ...
        .eq("id", leadId);
      if (leadClientId)
        recordingQuery = recordingQuery.eq("client_id", leadClientId);

      const { data, error } = await recordingQuery.select("*").maybeSingle();

      if (error) throw error;
      if (!data) return sendApiError(res, 404, "Lead not found");
      ...
      console.error(
        "POST /api/clients/:id/leads/:leadId/call-recording error:",
        error,
      );
      return sendApiError(
        res,
        500,
        error.message || "Failed to upload call recording",
      );
    }
  },
);
...
// ---------------------------------------------------------------------------
app.post("/api/clients/:id/leads", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const clientId = Number(req.params.id);
    ...
    const business = await resolveClientLeadBusiness(orgId, clientId);
    const lead = await createBusinessLead({
      orgId,
      business,
      body,
    });
    ...
    await insertClientActivityLog({
      ...
      entityType: "client_leads",
      entityId: lead.id,
      newValue: lead,
    });

    return sendApiSuccess(res, lead);
  } catch (error) {
    console.error("POST /api/clients/:id/leads error:", error);
    return sendApiError(
      res,
      error.statusCode || 500,
      error.message || "Failed to create lead",
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const business = await resolveClientLeadBusiness(orgId, clientId);
      const lead = await getBusinessLeadById({
        orgId,
        business,
        leadId,
      });
      ...
      console.error("GET /api/clients/:id/leads/:leadId error:", error);
      return sendApiError(res, 500, "Failed to load lead");
    }
  },
);
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      // A client mapped to an inline static business (e.g. Rasset) writes to
      // that business's own table with no client_id filter; everyone else uses
      // their per-client client_leads rows (filtered by client_id).
      const business = await resolveClientLeadBusiness(orgId, clientId);
      const { tableName: leadTable, clientId: leadClientId } =
        resolveLeadSource(business);
      ...
      // Lightweight inline updates (stage dropdown, client-visible toggle,
      // outreach/demo status dropdowns).
      if (isLightPatch) {
        ...
        if (body.pipeline_stage !== undefined) {
          ...
          const validStages = CLIENT_LEAD_PIPELINE_STAGES.map((s) => s.key);
          if (!validStages.includes(stage)) {
            return sendApiError(res, 400, "Invalid pipeline stage");
          }
          lightPatch.pipeline_stage = stage;
        }
        if (body.outreach_status !== undefined) {
          const outreach = normalizeText(body.outreach_status || "");
          const validOutreach = CLIENT_LEAD_OUTREACH_STATUSES.map((s) => s.key);
          if (!validOutreach.includes(outreach)) {
            return sendApiError(res, 400, "Invalid outreach status");
          }
          lightPatch.outreach_status = outreach;
        }
        if (body.demo_status !== undefined) {
          const demo = normalizeText(body.demo_status || "");
          const validDemo = CLIENT_LEAD_DEMO_STATUSES.map((s) => s.key);
          if (!validDemo.includes(demo)) {
            return sendApiError(res, 400, "Invalid demo status");
          }
          lightPatch.demo_status = demo;
        }
        if (body.callback_date !== undefined) {
          ...
          if (callbackDate && !/^\d{4}-\d{2}-\d{2}$/.test(callbackDate)) {
            return sendApiError(res, 400, "Invalid callback date");
          }
          lightPatch.callback_date = callbackDate || null;
        }
        if (body.assigned_to !== undefined) {
          lightPatch.assigned_to = String(body.assigned_to || "").trim() || null;
        }
        // Bulk "Assign for Phone" / "Assign for Email" send only their own
        // column, so they must stay on the light path — the full edit engine
        // requires phone/company/website/maps/yelp and would 400 on them.
        if (body.phone_assigned_to !== undefined) {
          lightPatch.phone_assigned_to =
            String(body.phone_assigned_to || "").trim() || null;
        }
        if (body.email_assigned_to !== undefined) {
          lightPatch.email_assigned_to =
            String(body.email_assigned_to || "").trim() || null;
        }
        if (body.category_type !== undefined) {
          const categoryType = normalizeText(body.category_type || "");
          const validCategoryTypes = CLIENT_LEAD_CATEGORY_TYPES.map((c) => c.key);
          if (categoryType && !validCategoryTypes.includes(categoryType)) {
            return sendApiError(res, 400, "Invalid category type");
          }
          lightPatch.category_type = categoryType || null;
        }
        if (body.is_client_visible !== undefined) {
          lightPatch.is_client_visible =
            body.is_client_visible === true ||
            body.is_client_visible === "true";
        }
        if (body.is_starred !== undefined) {
          lightPatch.is_starred =
            body.is_starred === true || body.is_starred === "true";
        }
        if (body.is_call_made !== undefined) {
          // Saving the quick-update popup clears the "call made" lock so the
          // call icon becomes actionable again.
          lightPatch.is_call_made =
            body.is_call_made === true || body.is_call_made === "true";
        }
        if (body.call_recording_url !== undefined) {
          // Allows clearing a recording by sending null/"".
          lightPatch.call_recording_url =
            String(body.call_recording_url || "").trim() || null;
        }
        if (body.notes !== undefined) {
          // Inline notes edit from the Leads table; empty clears the note.
          lightPatch.notes = String(body.notes || "").trim() || null;
        }
        if (body.add_note !== undefined && String(body.add_note).trim()) {
          // Append a timestamped, attributed note to the lead's notes history.
          const { data: current, error: readError } = await scopeLeadQuery(
            supabase.from(leadTable).select("notes"),
          ).maybeSingle();
          if (readError) throw readError;
          if (!current) return sendApiError(res, 404, "Lead not found");
          const authorName =
            req.loggedInUser?.name || req.loggedInUser?.email || null;
        ...
        // Capture previous status values so each change can be logged as an
        // explicit transition (powers the funnel metrics in the report tab).
        // Covers pipeline stage, outreach status and demo status.
        ...
        // One transition event per changed status dimension (field/from/to) for
        // funnel reporting (stage / outreach / demo movement, per team member).
        for (const field of changingFields) {
          const from = previousStatus[field] || null;
          const to = lightPatch[field];
          if (from === to) continue;
          await insertClientActivityLog({
            orgId,
            clientId,
      ...
      // Full edit via the shared engine. Read the tracked status values first
      // so an edit that also moves the stage records the same transition events
      // the inline Status dropdown does — otherwise the Status column's history
      // (and the funnel report) would silently miss form-driven changes.
      ...
      if (tableHasClientLeadColumns(leadTable)) {
        ...
        if (prevFullError) throw prevFullError;
        previousFullStatus = prevFull || {};
      }

      const lead = await updateBusinessLead({
        orgId,
        business,
        leadId,
        body,
      ...
      for (const field of [
        ...
        const from = previousFullStatus[field] || null;
        const to = (lead || {})[field] || null;
      ...
      console.error("PATCH /api/clients/:id/leads/:leadId error:", error);
      return sendApiError(
        res,
        error.statusCode || 500,
        error.message || "Failed to update lead",
...
app.post(
  "/api/clients/:id/leads/:leadId/note-audio",
  requireDashboardAuth,
  upload.single("audio"),
  async (req, res) => {
    try {
      ...
      if (!req.file || !req.file.buffer) {
        return sendApiError(res, 400, "No audio file uploaded");
      }
      ...
      const { data: current, error: readError } = await scopeLeadQuery(
        supabase
          .from(leadTable)
          .select("notes, pipeline_stage, demo_status"),
      ).maybeSingle();
      if (readError) throw readError;
      if (!current) return sendApiError(res, 404, "Lead not found");

      // Optional status/demo change carried alongside the voice note (the
      // Status Change Note and Demo Status Note modals reuse this flow). Same
      // validation as the PATCH light-patch path.
      const statusPatch = {};
      if (req.body?.pipeline_stage !== undefined) {
        const stage = normalizeText(req.body.pipeline_stage || "");
        const validStages = CLIENT_LEAD_PIPELINE_STAGES.map((s) => s.key);
        if (!validStages.includes(stage)) {
          return sendApiError(res, 400, "Invalid pipeline stage");
        }
      ...
      if (req.body?.demo_status !== undefined) {
        const demo = normalizeText(req.body.demo_status || "");
        const validDemo = CLIENT_LEAD_DEMO_STATUSES.map((s) => s.key);
        if (!validDemo.includes(demo)) {
          return sendApiError(res, 400, "Invalid demo status");
        }
      ...
      if (req.body?.callback_date !== undefined) {
      ...
      // Optional reached-via channel changes carried alongside the voice note
      // (the quick-update modal edits these together with status/demo).
      REACH_VIA_CHANNELS.forEach((c) => {
        if (req.body?.[c.column] !== undefined) {
          statusPatch[c.column] =
            req.body[c.column] === true || req.body[c.column] === "true";
        }
      });
      // Saving the quick-update popup clears the "call made" lock.
      if (req.body?.is_call_made !== undefined) {
        statusPatch.is_call_made =
          req.body.is_call_made === true || req.body.is_call_made === "true";
      }

      // 1) Store the audio so it can be played back from the notes later.
      ...
      // 2) Transcribe (best effort — a transcription failure must not block
      // saving the note + audio).
      let transcript = "";
      try {
        transcript = (
      ...
      // 3) Compose the note text from the typed note + transcription.
      ...
      const authorName =
        req.loggedInUser?.name || req.loggedInUser?.email || null;
      const notes = appendLeadNote(current.notes, noteText, authorName, {
        audio_url: audioUrl,
      });
      ...
      const { data, error } = await scopeLeadQuery(
        supabase.from(leadTable).update(updatePatch),
      )
        .select("*")
        .maybeSingle();
      if (error) throw error;
      ...
      // One transition event per changed status dimension so the voice-note
      // path feeds the funnel metrics exactly like the text-only PATCH path.
      ...
      console.error(
        "POST /api/clients/:id/leads/:leadId/note-audio error:",
        error,
      );
      return sendApiError(
        res,
        500,
        error.message || "Failed to save voice note",
      );
    }
  },
);
...
app.post(
  "/api/clients/:id/leads/:leadId/log-call",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      ...
      const { data: current, error: readError } = await scopeLeadQuery(
        supabase
          .from(leadTable)
          .select("is_call_made, call_time, call_made_by"),
      ).maybeSingle();
      if (readError) throw readError;
      if (!current) return sendApiError(res, 404, "Lead not found");

      // Already logged — return as-is so a double click / stale page is a no-op.
      if (current.is_call_made) {
        return sendApiSuccess(res, current);
      }

      const callMadeBy =
        req.loggedInUser?.name || req.loggedInUser?.email || "Unknown";
      const patch = {
        is_call_made: true,
      ...
      const { data, error } = await scopeLeadQuery(
        supabase.from(leadTable).update(patch),
      )
        .select("is_call_made, call_time, call_made_by")
        .maybeSingle();
      if (error) throw error;
      ...
      await insertClientActivityLog({
        ...
        entityType: leadTable,
        entityId: leadId,
        newValue: { call_time: patch.call_time, call_made_by: callMadeBy },
      });

      return sendApiSuccess(res, data);
    } catch (error) {
      console.error("POST /api/clients/:id/leads/:leadId/log-call error:", error);
      return sendApiError(res, 500, error.message || "Failed to log call");
    }
  },
);
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      return sendApiSuccess(res, { id: leadId });
    } catch (error) {
      console.error("DELETE /api/clients/:id/leads/:leadId error:", error);
      return sendApiError(
        res,
        error.statusCode || 500,
...
app.patch(
  "/api/clients/:id/linked-tasks/:taskId",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      ...
      if (!clientId || !taskId) {
        return sendApiError(res, 400, "Invalid client or task id");
      }

      // Authorize: the task must actually be linked to this client — its
      // free-text `business` must equal the client's name or company name (the
      // same rule the Linked Tasks list uses to surface it).
      const { data: client, error: clientError } = await supabase
        .from("clients")
        .select("name, company_name")
        .eq("org_id", orgId)
      ...
      const { data: task, error: taskError } = await supabase
        ...
        .maybeSingle();
      if (taskError) throw taskError;
      if (!task) return sendApiError(res, 404, "Task not found");
      ...
      if (
        !clientNameKeys.has(String(task.business || "").trim().toLowerCase())
      ) {
        return sendApiError(res, 403, "Task is not linked to this client");
      }
      ...
      if (body.status !== undefined) {
        ...
        patch.status = status;
        // Mirror the command system's status side effects.
        if (status === "done") patch.progress = 100;
        if (status === "open" && Number(task.progress) === 100)
          patch.progress = 0;
        if (task.blocker_note && status !== "blocked")
          patch.blocker_note = null;
        historyCalls.push([
          "status_change",
          "status",
      ...
      if (body.progress !== undefined) {
        ...
        if (!Number.isFinite(progress)) {
          return sendApiError(res, 400, "Invalid progress");
        }
      ...
      // Visibility toggle: whether this task shows on the client's external
      // dashboard. Not a task-content change, so it is not logged to history.
      if (body.is_client_visible !== undefined) {
      ...
      if (historyCalls.length === 0 && body.is_client_visible === undefined) {
        return sendApiError(res, 400, "No editable fields provided");
      }
      ...
      const { data: updated, error: updateError } = await supabase
        ...
        .maybeSingle();
      if (updateError) throw updateError;
      if (!updated) return sendApiError(res, 404, "Task not found");
      ...
      return sendApiSuccess(res, updated);
    } catch (error) {
      console.error(
        "PATCH /api/clients/:id/linked-tasks/:taskId error:",
        error,
      );
      return sendApiError(res, 500, error.message || "Failed to update task");
    }
  },
);
...
// Per-client Blockers
// ---------------------------------------------------------------------------
app.post("/api/clients/:id/blockers", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const clientId = Number(req.params.id);
    ...
    const title = String(body.title || "").trim();
    if (!clientId || !title) {
      return sendApiError(res, 400, "Blocker title is required");
    }

    const blockerSide = ["internal", "client_side"].includes(body.blocker_side)
      ? body.blocker_side
      : "internal";
    ...
    console.error("POST /api/clients/:id/blockers error:", error);
    return sendApiError(res, 500, "Failed to create blocker");
  }
});

app.patch(
  "/api/clients/:id/blockers/:blockerId",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      ...
      if (!clientId || !blockerId) {
        return sendApiError(res, 400, "Invalid client or blocker id");
      }
      ...
      if (existingError) throw existingError;
      if (!existing) return sendApiError(res, 404, "Blocker not found");
      ...
      if (body.blocker_side !== undefined) {
        if (!["internal", "client_side"].includes(body.blocker_side)) {
          return sendApiError(res, 400, "Invalid blocker side");
        }
      ...
      if (body.resolution_status !== undefined) {
        if (
          !["open", "in_progress", "resolved"].includes(body.resolution_status)
        ) {
          return sendApiError(res, 400, "Invalid resolution status");
        }
      ...
      console.error("PATCH /api/clients/:id/blockers/:blockerId error:", error);
      return sendApiError(res, 500, "Failed to update blocker");
    }
  },
);
...
app.post("/api/ai/parse-meeting-notes", requireDashboardAuth, async (req, res) => {
  try {
    const notes = String(req.body?.notes || "").trim();

    if (!notes) {
      return sendApiError(res, 400, "Meeting details are required");
    }
...
`;
    ...
    console.error("POST /api/ai/parse-meeting-notes error:", error);
    return sendApiError(res, 500, "Failed to auto-fill meeting from notes");
  }
});

app.post("/api/clients/:id/meetings", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const clientId = Number(req.params.id);
    ...
    const payload = buildClientMeetingPayloadFromBody(body);
    if (!payload.title && !payload.meeting_date) {
      return sendApiError(res, 400, "Meeting title or date is required");
    }
    ...
    return sendApiError(res, 500, "Failed to create meeting");
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (!clientId || !meetingId) {
        return sendApiError(res, 400, "Invalid client or meeting id");
      }
      ...
      if (error) throw error;
      if (!data) return sendApiError(res, 404, "Meeting not found");

      return sendApiSuccess(res, data);
    } catch (error) {
      console.error("GET /api/clients/:id/meetings/:meetingId error:", error);
      return sendApiError(res, 500, "Failed to load meeting");
    }
  },
);
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (existingError) throw existingError;
      if (!existing) return sendApiError(res, 404, "Meeting not found");
      ...
      if (body.archive === true) {
        ...
        await insertClientActivityLog({
          ...
          clientId,
          actorUserId,
          action: "client_meeting_archived",
          entityType: "client_meetings",
          entityId: meetingId,
          oldValue: existing,
          newValue: data,
      ...
      console.error("PATCH /api/clients/:id/meetings/:meetingId error:", error);
      return sendApiError(res, 500, "Failed to update meeting");
    }
  },
);
...
app.post("/api/clients/:id/campaigns", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const clientId = Number(req.params.id);
    ...
    if (!payload.name) return sendApiError(res, 400, "Campaign name is required");
    ...
    console.error("POST /api/clients/:id/campaigns error:", error);
    return sendApiError(res, 500, "Failed to create campaign");
  }
});

app.patch(
  "/api/clients/:id/campaigns/:campaignId",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      ...
      if (!clientId || !campaignId)
        return sendApiError(res, 400, "Invalid client or campaign id");
      ...
      if (!existing) return sendApiError(res, 404, "Campaign not found");
      ...
      await insertClientActivityLog({
        ...
        clientId,
        actorUserId,
        action: body.archive ? "client_campaign_archived" : "client_campaign_updated",
        entityType: "client_campaigns",
        entityId: campaignId,
        oldValue: existing,
        newValue: data,
      ...
      console.error("PATCH /api/clients/:id/campaigns/:campaignId error:", error);
      return sendApiError(res, 500, "Failed to update campaign");
    }
  },
);
...
app.post("/api/clients/:id/incentives", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const clientId = Number(req.params.id);
    ...
    const payload = buildIncentivePayloadFromBody(body);
    if (!payload.title) return sendApiError(res, 400, "Incentive title is required");
    ...
    console.error("POST /api/clients/:id/incentives error:", error);
    return sendApiError(res, 500, "Failed to create incentive");
  }
});

app.patch(
  "/api/clients/:id/incentives/:incentiveId",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      const actorUserId = req.loggedInUser?.id || null;
      const clientId = Number(req.params.id);
      const incentiveId = Number(req.params.incentiveId);
      const body = req.body || {};
      if (!clientId || !incentiveId)
        return sendApiError(res, 400, "Invalid client or incentive id");
      ...
      if (!existing) return sendApiError(res, 404, "Incentive not found");
      ...
      console.error("PATCH /api/clients/:id/incentives/:incentiveId error:", error);
      return sendApiError(res, 500, "Failed to update incentive");
    }
  },
);
...
app.post("/api/clients/:id/reports", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const clientId = Number(req.params.id);
    ...
    if (!payload.period_label && !payload.week_start && !payload.summary) {
      return sendApiError(res, 400, "Report period or summary is required");
    }
    ...
    await insertClientActivityLog({
      ...
      clientId,
      actorUserId,
      action: "client_weekly_report_created",
    ...
    console.error("POST /api/clients/:id/reports error:", error);
    return sendApiError(res, 500, "Failed to create weekly report");
  }
});

app.patch(
  "/api/clients/:id/reports/:reportId",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      ...
      if (!clientId || !reportId)
        return sendApiError(res, 400, "Invalid client or report id");
      ...
      if (!existing) return sendApiError(res, 404, "Report not found");

      let patch;
      if (body.archive === true) {
        patch = { is_active: false, deleted_at: new Date().toISOString() };
      } else if (body.publish === true) {
        patch = { is_published: true, published_at: new Date().toISOString() };
      ...
      await insertClientActivityLog({
        ...
        clientId,
        actorUserId,
        action: "client_weekly_report_updated",
        entityType: "client_weekly_reports",
        entityId: reportId,
        oldValue: existing,
        newValue: data,
      ...
      console.error("PATCH /api/clients/:id/reports/:reportId error:", error);
      return sendApiError(res, 500, "Failed to update weekly report");
    }
  },
);
...
app.post("/api/clients", requireDashboardAuth, async (req, res) => {
  try {
    ...
    // Slug comes from the client name now (the create form has no slug field),
    // so retry with a numeric suffix instead of bouncing the user on collisions.
    let client = null;
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
      ...
      if (clientError.code !== "23505" || attempt === 5) {
...
app.get("/api/team-work", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    let date = String(req.query.date || "").trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      date = getTodayDateStringInTimeZone();
    }
    const data = await loadTeamWorkData(orgId, date);
    const logs = await getRecentTeamWorkLogs(orgId, 40);
    return sendApiSuccess(res, { ...data, logs });
  } catch (error) {
    console.error("GET /api/team-work error:", error);
    return sendApiError(res, 500, "Failed to load team work data");
  }
});

app.get("/api/team-work/logs", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const logs = await getRecentTeamWorkLogs(orgId, 40);
...
app.post("/api/team-work/hours", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const actorName = req.loggedInUser?.name || "";
    const body = req.body || {};
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !memberId || !columnId) {
      return sendApiError(res, 400, "Invalid date, member or column");
    }
    if (isNaN(hours) || hours < 0) hours = 0;
    if (hours > 24) hours = 24;

    // Look up member/column labels + the previous value (for the log).
    ...
    console.error("POST /api/team-work/hours error:", error);
    return sendApiError(res, 500, "Failed to save hours");
  }
});

app.post("/api/team-work/members", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const actorName = req.loggedInUser?.name || "";
    ...
    if (!name) return sendApiError(res, 400, "Name is required");

    // Append to the end of that team.
    ...
    console.error("POST /api/team-work/members error:", error);
    return sendApiError(res, 500, "Failed to add person");
  }
});
...
app.patch("/api/team-work/members/:id", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const memberId = Number(req.params.id);
    const body = req.body || {};
    if (!memberId) return sendApiError(res, 400, "Invalid id");
    ...
    console.error("PATCH /api/team-work/members/:id error:", error);
    return sendApiError(res, 500, "Failed to update person");
  }
});

app.delete(
  "/api/team-work/members/:id",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      ...
      return sendApiSuccess(res, { id: memberId });
    } catch (error) {
      console.error("DELETE /api/team-work/members/:id error:", error);
      return sendApiError(res, 500, "Failed to remove person");
    }
  },
);

app.post("/api/team-work/columns", requireDashboardAuth, async (req, res) => {
  try {
    const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
    const actorUserId = req.loggedInUser?.id || null;
    const actorName = req.loggedInUser?.name || "";
    const label = String((req.body || {}).label || "").trim();
    if (!label) return sendApiError(res, 400, "Label is required");
    ...
    const { data, error } = await supabase
      .from("team_work_columns")
      .insert([
        { org_id: orgId, label, sort_order: sortOrder, is_active: true },
      ])
      .select("id, label")
      .maybeSingle();
    if (error) throw error;
    ...
    return sendApiSuccess(res, { id: data.id, label: data.label });
  } catch (error) {
    console.error("POST /api/team-work/columns error:", error);
    return sendApiError(res, 500, "Failed to add column");
  }
});

app.delete(
  "/api/team-work/columns/:id",
  requireDashboardAuth,
  async (req, res) => {
    try {
      const orgId = req.loggedInUser?.org_id || DASHBOARD_ORG_ID;
      ...
      if (!columnId) return sendApiError(res, 400, "Invalid id");
      ...
      return sendApiSuccess(res, { id: columnId });
    } catch (error) {
      console.error("DELETE /api/team-work/columns/:id error:", error);
      return sendApiError(res, 500, "Failed to remove column");
    }
  },
);
...
// Served by Next.js (next start) instead of app.listen(). The route registry
