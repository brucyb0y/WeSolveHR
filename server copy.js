...
function parseFlexibleDateText(input) {
  ...
  const base = getLocalStartOfDay(new Date());

  if (!text) return null;

  if (text === "today") {
    return formatLocalDateForDb(base);
  }

  if (text === "tomorrow") {
    const d = new Date(base);
    d.setDate(d.getDate() + 1);
    return formatLocalDateForDb(d);
  }
  ...
  if (match) {
    ...
    if (month && day >= 1 && day <= 31) {
      const d = buildDateForCurrentYear(month, day);
      if (d) return formatLocalDateForDb(d);
    }
  }
...
function parseOffDayCommand(text) {
  ...
  const offDateText = match[2].trim().replace(/^on\s+/i, "").trim();
...
async function getTodayAttendanceEventsForAllUsers() {
  const start = getLocalStartOfDay().toISOString();
...
async function handleNowSummary(res, actingUser) {
  ...
  try {
    ...
    const loggedIn = [];
    const onBreak = [];
    const onLeave = plannedOff.map((x) => x.users?.name || "Unknown");
    const loggedOut = [];
    const noUpdate = [];
    ...
    function getBreakStats(userEvents) {
      ...
      for (let i = 0; i < userEvents.length; i += 1) {
        ...
        if (matchedBack) {
          ...
          const diffMin = Math.max(
            0,
            Math.round((now - breakStart) / 60000),
          );
          totalBreakMinutes += diffMin;
    ...
    for (const user of users) {
      ...
      if (!latest) {
        noUpdate.push(user.name);
        continue;
      }

      if (latest.action === "break") {
        onBreak.push(
          `${user.name} (current break: ${formatMinutesShort(
            breakStats.currentBreakMinutes,
          )} | total break today: ${formatMinutesShort(
            breakStats.totalBreakMinutes,
          )})`,
        );
        continue;
      }
      ...
      if (latest.action === "logout") {
        loggedOut.push(
          `${user.name} (break used: ${formatMinutesShort(
            breakStats.totalBreakMinutes,
          )})`,
        );
        continue;
    ...
    const lines = [
      "📋 Now summary",
      `Total team: ${totalTeam} | Working: ${loggedIn.length} | Break: ${onBreak.length} | Leave: ${onLeave.length} | Logged out: ${loggedOut.length} | No update: ${noUpdate.length}`,
      "",
      "✅ Logged in:",
      loggedIn.length ? loggedIn.join(", ") : "None",
      "",
      "☕ On break:",
      onBreak.length ? onBreak.join(", ") : "None",
      "",
      "🌴 On leave today:",
      onLeave.length ? onLeave.join(", ") : "None",
      "",
      "🏁 Logged out today:",
      loggedOut.length ? loggedOut.join(", ") : "None",
      "",
      "❓ No attendance marked today:",
      noUpdate.length ? noUpdate.join(", ") : "None",
    ];
...
async function getDashboardSummaryData() {
  const today = formatLocalDateForDb(getLocalStartOfDay(new Date()));
...
async function getAttendancePageData() {
  ...
  const activeTodayUserIds = new Set(
    (events || [])
      .filter((e) => String(e.created_at || "").startsWith(today))
      .map((e) => e.user_id),
  );
